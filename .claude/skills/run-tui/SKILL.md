...
2. **Pick the exe path**. Debug build output:
   ```
   src/PT.Tui/bin/Debug/net10.0-windows/pt-tui.exe
   ```
...
```
and launch from `src/PT.Tui/bin/Release/net10.0-windows/win-arm64/publish/pt-tui.exe` instead. Otherwise stick with Debug — it builds in seconds and the TUI is not CPU-bound.
